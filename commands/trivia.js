...
module.exports = {
	...
	execute(message) {
		...
		async function runTrivia(trivia) {
			...
			// console.log(attempts, correctAnswer, correctNumber);
			...
			// if not a dm, make stealing possible
			...
			collector.on('collect', m => {
				...
				if (time != 10 && m.author.id != message.author.id) {
					...
					if (text === 'cancel') {message.channel.send('Recieved \'cancel\': Game terminated.'); return answered = true;}
