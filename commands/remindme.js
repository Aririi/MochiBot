	...
	execute(message, args, client, timeDB) {
		...
		if (args[0].search(timeRegex) != -1) {
			...
			timeDB.insert(reminder, function(err) {
				if (err) {return console.error('DB ERROR: Failed to make new reminder:', err);}
		...
		function sendReminder() {
			...
			timeDB.find({ time: { $gte: timeNow - 2 } }, function(err, docs) {
				if (err) {message.channel.send('There was an error trying to find a reminder for this time.'); return console.error('DB ERROR: Failed to search for reminder:', err);}
				...
				if (docs[0] != undefined) {
					...
					timeDB.remove({ _id: docs[0]._id }, {}, function(err) {
						if (err) {message.channel.send('There was an error removing the most recent reminder.'); return console.error('DB ERROR: Failed to remove sent reminder:', err);}
					});
				}
			});
		}
