...
module.exports = {
	...
	execute(message, args) {
		...
		async function randomAvatar() {
			...
			await nekolife.sfw.avatar().then(result => sendEmbed(description, result.url));
		}
		async function randomCat() {
			const description = 'Here\'s a random cat image/GIF.';
			await nekolife.sfw.meow().then(result => sendEmbed(description, result.url));
		}
		async function randomDog() {
			const description = 'Here\'s a random dog image/GIF.';
			await nekolife.sfw.woof().then(result => sendEmbed(description, result.url));
		}
		async function randomFoxgirl() {
			const description = 'Here\'s a random foxgirl image.';
			await nekolife.sfw.foxGirl().then(result => sendEmbed(description, result.url));
		}
		async function randomGoose() {
			const description = 'Here\'s a random goose image.';
			await nekolife.sfw.goose().then(result => sendEmbed(description, result.url));
		}
		async function randomGECG() {
			const description = 'Here\'s a random GECG (Genetically Engineered CatGirl) image.';
			await nekolife.sfw.gecg().then(result => sendEmbed(description, result.url));
		}
		async function randomHolo() {
			const description = 'Here\'s a random foxgirl image.';
			await nekolife.sfw.holo().then(result => sendEmbed(description, result.url));
		}
		async function randomKemonomimi() {
			const description = 'Here\'s a random kemonomimi image.';
			await nekolife.sfw.kemonomimi().then(result => sendEmbed(description, result.url));
		}
		async function randomLizard() {
			const description = 'Here\'s a random lizard image.';
			await nekolife.sfw.lizard().then(result => sendEmbed(description, result.url));
		}
		async function randomNeko() {
			const description = 'Here\'s a random neko image.';
			await nekolife.sfw.neko().then(result => sendEmbed(description, result.url));
		}
		async function randomNekoGIF() {
			const description = 'Here\'s a random neko GIF.';
			await nekolife.sfw.nekoGif().then(result => sendEmbed(description, result.url));
		}
		async function randomWaifu() {
			const description = 'Here\'s a random waifu image.';
			await nekolife.sfw.waifu().then(result => sendEmbed(description, result.url));
		}
		...
		if (isNaN(args[1]) === false) {
			if (args[1] > 8 && message.author.id != owner) {return message.channel.send('That\'s too many.');}
			else if (args[1] == 0) {return message.channel.send('Sending no random media.');}
			else {
				let loopCount = args[1];
				while (loopCount > 0) {
					setTimeout(randomTypeCheck, 2000);
